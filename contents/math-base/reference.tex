...
\paragraph*{矢量操作}
\begin{enumerate}
    \item \textit{矢量加法}：矢量$\bm{B}$的尾叠在矢量$\bm{A}$的头上。一般表现为三角形
    ...
    \item \textit{矢量与矢量点积}：两矢量点积定义为
           ...
           \begin{align}
               ...
               \bm{A} \cdot (\bm{B} + \bm{C}) &= \bm{A} \cdot \bm{B} + \bm{A} 
                                                 \cdot \bm{B}
    ...
    \item \textit{矢量与矢量叉积}：两矢量叉积定义为
          ...
          \end{equation}
          $\bm{A}$与$\bm{B}$的叉积可理解为边长为$A$和$B$的平行四边形的面积，在乘上此
          四边形法向的单位向量。
\end{enumerate}
...
\end{equation}
直角坐标系中任一矢量都可由这三个单位矢量经线性组合构成，进而定义矢量与矢量、矢量与标
量间的运算规则。
...
定义$\nabla$算符为
\begin{equation}
    \nabla =    \vec{\bm{e}}_x \frac{\partial T}{\partial x} 
               +\vec{\bm{e}}_y \frac{\partial T}{\partial y}
               +\vec{\bm{e}}_z \frac{\partial T}{\partial z}
...
\begin{equation}
    \nabla T =   \frac{\partial T}{\partial x}\vec{\bm{e}}_x 
               + \frac{\partial T}{\partial y}\vec{\bm{e}}_y
               + \frac{\partial T}{\partial z}\vec{\bm{e}}_z
...
散度是一个标量
\begin{equation}
    \nabla \cdot \vec{v} = (   \vec{\bm{e}}_x \frac{\partial T}{\partial x} 
                             + \vec{\bm{e}}_y \frac{\partial T}{\partial y}
                             + \vec{\bm{e}}_z \frac{\partial T}{\partial z})
                           \cdot
...
们可将$\vec{v}$当成一个不可压缩的流，$\vec{v}$的通量是单位时间内流出表面的总流量
(式\eqref{eq:div-basetheo}左边)。某点处$\vec{v}$的散度表示该点产生或吸收流的量度(散度
为正表示放出流，散度为负表示吸收流)，对于空间中某一区域，其在单位时间内产生(或吸收)的
流等于流经其表面的通量。
...
方向沿径向、极角和方位角。需要注意的是，这些单位矢量随着位置变化的。
\subsection{球坐标下的$\nabla$算符}
球坐标系下的$\nabla$算符可写为：
\begin{equation}
    \nabla =  \vec{\bm{e}}_r \frac{\partial}{\partial r} 
            + \vec{\bm{e}}_\theta \frac{\partial}{\partial \theta}
            + \vec{\bm{e}}_\phi \frac{1}{r\sin{\theta}} \frac{\partial}{\partial \phi}
    \label{eq:nabla-sphere-form}
\end{equation}

球坐标系下标量场$T$和矢量场$\vec{\bm{v}}$，矢量导数的坐标表示为，\newline
梯度：
\begin{equation}
    \nabla T = \frac{\partial T}{\partial r} \vec{\bm{e}}_r
              +\frac{1}{r} \frac{\partial T}{\partial \theta} \vec{\bm{e}}_{\theta}
              +\frac{1}{r\sin{\theta}}\frac{\partial T}{\partial \phi} \vec{\bm{e}}_{\phi}
    \label{eq:sphere-gradient}
\end{equation}
散度：
\begin{equation}
    \nabla \cdot \vec{\bm{v}} = \frac{1}{r^2} \frac{\partial}{\partial r}(r^2 v_r)
...
\begin{equation}
    ...
    \frac{1}{r\sin{\theta}}\left[ \frac{\partial}{\partial\theta}(\sin{\theta}\,v_{\phi}) 
                      - \frac{\partial v_{\theta}}{\partial\phi}\right] \vec{\bm{e}}_r
     +\frac{1}{r} \left[ \frac{1}{\sin{\theta}} \frac{\partial v_{r}}{\partial \phi} 
                 - \frac{\partial}{\partial r}(r v_{\phi})\right] \vec{\bm{e}}_{\theta}
     +\frac{1}{r}\left[\frac{\partial}{\partial r}(r v_{\theta})
                 - \frac{\partial v_{r}}{{\partial \theta}} \right] \vec{\bm{e}}_{\phi}
...
\begin{proof}
    必要性，上面左边等式
    \begin{equation*}
        \vec{\bm{F}} = - \nabla V = -\vec{\bm{e}}_x \frac{\partial V_x}{\partial x}
                              -\vec{\bm{e}}_y \frac{\partial V_y}{\partial y}
                              -\vec{\bm{e}}_z \frac{\partial V_z}{\partial z}
    \end{equation*}
    因此有
    \begin{equation*}
        \nabla \times \vec{\bm{F}} = \left|\begin{matrix}
            \vec{\bm{e}}_x  &   \vec{\bm{e}}_y  &   \vec{\bm{e}}_z  \\
